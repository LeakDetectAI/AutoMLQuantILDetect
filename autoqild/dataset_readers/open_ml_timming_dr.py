    ...
    **kwargs : dict
    ...
    def __clean_up_dataset__(self):
        ...
        for column in categorical_columns:
            if column != LABEL_COL:
                self.data_frame_raw[column] = label_encoder.fit_transform(
                    self.data_frame_raw[column]
                )

        self.data_frame_raw[LABEL_COL] = self.data_frame_raw[LABEL_COL].apply(
    ...
    def get_sampled_imbalanced_data(self, X, y):
        ...
        if self.imbalance < 0.5:
            ...
            self.logger.info(
                f"Before processing----ratio {n_1 / n_0} p {self.imbalance}, n_0 {n_0}, n_1 {n_1}----"
            )
            ind0 = np.where(y == 0)[0]
            ...
            self.logger.info(
                f"After processing----ratio {n_1 / n_0} p {self.imbalance}, n_0 {n_0}, n_1 {n_1}----"
            )
        return X, y
